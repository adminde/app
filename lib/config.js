var config = {
    ...
    init: function() {
        ...
        for (var z in config.devices) config.devicesbyname[config.devices[z].name] = config.devices[z];
        for (var z in config.devices) config.devicesbyid[config.devices[z].id] = config.devices[z];
    ...
    UI: function() {
        ...
        for (var z in config.app) {
            ...
            } else if (config.app[z].type=="device") {
                out += "<i class='status icon-ok-sign icon-white'></i> <b class='devicename' key='"+z+"'>"+config.app[z].autoname+" <span style='color:#ccc; font-size:12px'>[AUTO]</span></b><i class='appconfig-edit icon-pencil icon-white' style='float:right; cursor:pointer'></i>";
                out += "<br><span class='appconfig-info'></span>";
                out += "<div class='device-select-div input-append'><select id='device-selection' class='device-select'></select><button class='btn device-select-ok'>ok</button></div>";
        ...
        for (var z in config.app) {
            ...
            if (config.app[z].type=="device") {
                // Create list of devices
                var out = "<option value=0>Select "+z+" device:</option>";
                out += "<option value=auto>AUTO SELECT</option>";
                for (var n in config.devicesbyname)  {
                    out += "<option value="+config.devicesbyname[n].id+">"+config.devicesbyname[n].name+"</option>";
                }
                configItem.find(".device-select").html(out);
                
                var devicevalid = false;
                // Check for existing configuration
                if (config.db[z]!=undefined) {
                    var deviceid = config.db[z];
                    if (config.devicesbyid[deviceid]!=undefined) {
                        var keyappend = ""; if (z!=config.devicesbyid[deviceid].name) keyappend = z+": ";
                        configItem.find(".devicename").html(keyappend+config.devicesbyid[deviceid].name);
                        configItem.find(".device-select").val(feedid);
                        configItem.find(".device-select-div").hide();
                        devicevalid = true;
                    } else {
                        // Invalid deviceid remove from configuration
                        delete config.db[z]; 
                    }
                }
                
                // Important that this is not an else here as an invalid deviceid 
                // in the section above will call delete making the item undefined again
                if (config.db[z]==undefined) {
                    // Auto match devices that follow the naming convention
                    for (var n in config.devicesbyid) {
                        if (config.devicesbyid[n].name==config.app[z].autoname) {
                            configItem.find(".device-select").val("auto");
                            configItem.find(".device-select-div").hide();
                            devicevalid = true;
                        }
                    }
        ...
        $(".appconfig-edit").click(function(){
            ...
            if (config.app[key].type=="device") {
            	configItem.find(".device-select-div").show();
            }
        });
        ...
        $(".device-select-ok").unbind("click");
        $(".device-select-ok").click(function(){
        	var key = $(this).parent().parent().attr("key");
        	var configItem = $(".appconfig-box[key="+key+"]");
        	
        	var deviceid = $(this).parent().find(".device-select").val();
        	
        	if (deviceid!="auto" && deviceid!=0) {
        		config.db[key] = deviceid;
        		var keyappend = ""; if (key!=config.devicesbyid[deviceid].name) keyappend = key+": ";
        		configItem.find(".devicename").html(keyappend+config.devicesbyid[deviceid].name);
        		configItem.find(".status").addClass("icon-ok-sign"); 
        		configItem.find(".status").removeClass("icon-remove-circle");
        		// Save config
        	...
        	if (deviceid=="auto") {
        		delete config.db[key];
        		configItem.find(".devicename").html(config.app[key].autoname+" <span style='color:#ccc; font-size:12px'>[AUTO]</span>");
        	}
        	...
        	var name = $('#device-selection option:selected').text();

        	var outputs = config.devicesbyname[name].output;
    ...
    load: function()
    {
        for (var key in config.app) {
            ...
            if (config.app[key].type=="device") {
            	...
            	// Check if devices match naming convention
            	var autoname = config.app[key].autoname;
            	if (config.devicesbyname[autoname]!=undefined) {
            		config.app[key].value = config.devicesbyname[autoname].id;
            	}
            	// Overwrite with any user set devices if applicable
            	if (config.db[key]!=undefined) {
            		config.app[key].value = config.db[key];
            	}
            }
