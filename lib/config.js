var config = {
    ...
    UI: function() {
        ...
        for (var z in config.app) {
            ...
            var type = config.app[z].type;
            
            if (type=="feed") {
            	config.createList(type, z, configItem);
            }
    ...
    createList: function(type, z, configItem)
    {
    	// Create list of items that satisfy engine requirement
        var out = "<option value=0>Select "+z+" " + type + ":</option>";
        out += "<option value=auto>AUTO SELECT</option>";
        for (var n in config[type + "sbyname"])  {
            if (config.engine_check(config[type + "sbyname"][n],config.app[z])) {
                out += "<option value="+config[type + "sbyname"][n].id+">"+config[type + "sbyname"][n].name+"</option>";
            }
        }
        configItem.find("." + type + "-select").html(out);
        
        var valid = false;
        // Check for existing configuration
        if (config.db[z]!=undefined) {
            var id = config.db[z];
            if (config[type + "sbyid"][id]!=undefined && config.engine_check(config[type + "sbyid"][id],config.app[z])) {
                var keyappend = ""; if (z!=config[type + "sbyid"][id].name) keyappend = z+": ";
                configItem.find("." + type + "name").html(keyappend+config[type + "sbyid"][id].name);
                configItem.find("." + type + "-select").val(id);
                configItem.find("." + type + "-select-div").hide();
                valid = true;
            } else {
                // Invalid id remove from configuration
                delete config.db[z]; 
            }
        }
        
        // Important that this is not an else here as an invalid id 
        // in the section above will call delete making the item undefined again
        if (config.db[z]==undefined) {
            // Auto match items that follow the naming convention
            for (var n in config[type + "sbyid"]) {
                if (config[type + "sbyid"][n].name==config.app[z].autoname && config.engine_check(config[type + "sbyid"][n],config.app[z])) {
                    configItem.find("." + type + "-select").val("auto");
                    configItem.find("." + type + "-select-div").hide();
                    valid = true;
                }
            }
