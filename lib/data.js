var data = {
    ...
    iteratePower: function(start, end, interval) {
    ...
    iterateDailyEnergy: function(start, end) {
            ...
            while (time <= end) {
                ...
                for (var key in self.series) {
                    ...
                    if (series instanceof FixedIntervalPowerSeries ||
                        ...
                        timevalue[key] = series.iterateTimevalue(time, interval, index);
                    }
                }
                yield timevalue;
        ...
        if (this.earliest == null) {
            if (typeof callback == 'function' || async) {
                ...
                return this.loadMeta(function(result) {
                    ...
                    var earliestValue = self.getTimevalue(earliestTime)[1];
...
class FixedIntervalEnergySeries extends FixedIntervalSeries {
    ...
    getDailyEnergy(time) {
        if (this.data.length > 0 && 
                this.data[0][0] <= time && 
                this.data[this.data.length-1][0] >= (time - 86400000)) {
            ...
            if (this.data[this.data.length-1][0] == time) {
                ...
                var pos = (time - this.data[0][0])/86400000;
                if (pos < this.data.length-1) {
                    return this.data[pos+1][1] - this.data[pos][1];
                }
            }
            return null;
        }
