...
<div id="app-setup" style="display:none; padding-top:50px" class="block">
    <h2 class="appconfig-title">Energy and power consumption</h2>
    ...
    <div class="appconfig-description">
        <div class="appconfig-description-inner">
            ...
            <br><br>
            <em>(This app is deprecated and shouldn't be used anymore)</em>
            <br><br>
...
function update() {
    ...
    data.update(function(result) {
        ...
        // --------------------------------------------------------------------------------------------------------
        var powerTime = result.getLatestTime("use");
        ...
        var energyTime = result.getLatestTime("use_kwh");
        ...
        if (reload) {
            ...
            data.loadPower(view.start, view.end, interval, function(result) {
                lastPowerTime = result.getLatestTime("use");
...
function drawPowerValues(power) {
    var value = power.getLatestValue("use");
    ...
    if (viewMode == "energy") {
        ...
        if (costNow < 1.0) {
            $("#powernow").html(config.app.currency.value+costNow.toFixed(3)+"/hr");
        }
        else {
            $("#powernow").html(config.app.currency.value+costNow.toFixed(2)+"/hr");
        }
    }
}

function drawPowerGraph(power) {
    var consPower = power.getData("use");
